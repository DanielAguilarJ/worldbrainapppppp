...
class StageManager: ObservableObject {
    ...
    private let finalExamPassedKey = "finalExamPassed" // NUEVO
    
    // NUEVO: Propiedad para saber si se debe mostrar el examen final
    @Published var showFinalExam: Bool = false
    @Published var finalExamStageIndex: Int = 0
    ...
    private func loadSavedProgress() {
        ...
        let passedExams = userDefaults.array(forKey: finalExamPassedKey) as? [Int] ?? [] // NUEVO
    ...
    private func saveProgress() {
        ...
        let passedExams = userDefaults.array(forKey: finalExamPassedKey) as? [Int] ?? [] // NUEVO: Recuperar exámenes pasados
        ...
        userDefaults.set(passedExams, forKey: finalExamPassedKey) // NUEVO: Guardar exámenes aprobados
    ...
    private func completeLessonByIndex(stageIndex: Int, lessonIndex: Int) {
        ...
        if !stages[stageIndex].lessons[lessonIndex].isCompleted {
            ...
            // NUEVO: Verificar si es la última lección de la etapa y mostrar el examen final
            if isLastLesson(stageIndex: stageIndex, lessonIndex: lessonIndex) &&
               completedLessonsCount >= stages[stageIndex].requiredLessons {
                // Activar el examen final
                showFinalExam = true
                finalExamStageIndex = stageIndex
                print("🎓 Se ha completado la última lección. Mostrando examen final para la etapa \(stageIndex)")
            }
            // Si no es la última lección, seguimos con el comportamiento normal
            else if completedLessonsCount >= stages[stageIndex].requiredLessons {
                print("🏆 ¡Etapa \(stageIndex) completada! Requisito cumplido: \(completedLessonsCount)/\(stages[stageIndex].requiredLessons)")
                
                // NOTA: Ya no desbloqueamos automáticamente la siguiente etapa
                // Ahora se desbloquea solo después de aprobar el examen final
            }
    ...
    // NUEVO: Verificar si es la última lección de la etapa
    func isLastLesson(stageIndex: Int, lessonIndex: Int) -> Bool {
        guard stageIndex < stages.count else { return false }
        return lessonIndex == stages[stageIndex].lessons.count - 1
    }
    
    // NUEVO: Método para desbloquear la siguiente etapa después del examen
    func unlockNextStage(afterStageIndex: Int) {
        let passedExams = userDefaults.array(forKey: finalExamPassedKey) as? [Int] ?? []
        
        // Verificar si hay una siguiente etapa para desbloquear
        if afterStageIndex + 1 < stages.count {
            if stages[afterStageIndex + 1].isLocked {
                print("🔓 Desbloqueando siguiente etapa: \(afterStageIndex + 1)")
                stages[afterStageIndex + 1].isLocked = false
                
                // Desbloquear primera lección de la siguiente etapa
                if !stages[afterStageIndex + 1].lessons.isEmpty {
                    print("🔓 Desbloqueando primera lección de la etapa \(afterStageIndex + 1)")
                    stages[afterStageIndex + 1].lessons[0].isLocked = false
                    // Asegurar que la primera lección NO está marcada como completada
                    stages[afterStageIndex + 1].lessons[0].isCompleted = false
                } else {
                    print("⚠️ La etapa \(afterStageIndex + 1) no tiene lecciones para desbloquear")
                }
                
                // Marcar el examen como aprobado
                var updatedPassedExams = passedExams
                if !updatedPassedExams.contains(afterStageIndex) {
                    updatedPassedExams.append(afterStageIndex)
                }
                userDefaults.set(updatedPassedExams, forKey: finalExamPassedKey)
                userDefaults.synchronize()
                
                print("🎓 Examen de etapa \(afterStageIndex) marcado como aprobado")
            } else {
    ...
    func resetProgress() {
        ...
        userDefaults.removeObject(forKey: finalExamPassedKey) // NUEVO: Resetear exámenes aprobados
        userDefaults.synchronize()
